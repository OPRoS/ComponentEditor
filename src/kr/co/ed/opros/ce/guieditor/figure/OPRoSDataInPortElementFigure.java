...
public class OPRoSDataInPortElementFigure extends LabeledFigure {
	...
	public Image getWidgetImage(){
		if(direction==1)
			icon = OPRoSUtil.createImage(OPRoSStrings.getString("DataInFigureE"));
		else if(direction==2)
			icon = OPRoSUtil.createImage(OPRoSStrings.getString("DataInFigureW"));
		else if(direction==3)
			icon = OPRoSUtil.createImage(OPRoSStrings.getString("DataInFigureS"));
		else if(direction==4)
			icon = OPRoSUtil.createImage(OPRoSStrings.getString("DataInFigureN"));
		return icon;
	}
	public OPRoSDataInPortElementFigure(){
		direction=4;
		layout = new XYLayout();
		setLayoutManager(layout);
		label = new Label();
		label.setForegroundColor(ColorConstants.darkGray);
		label.setFont(new Font(null,OPRoSStrings.getString("DataInFigureFont"),10,SWT.NONE));
